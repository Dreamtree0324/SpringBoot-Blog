...
<c:if test="${post.id == 0}"><c:url var="actionUrl" value="/post/write"></c:url></c:if>
<c:if test="${post.id != 0}"><c:url var="actionUrl" value="/post/${post.id}/edit"></c:url></c:if>
...
<form:form action="${actionUrl}" modelAttribute="post" onsubmit="if($('#pen').html()!='<p><br></p>')$('#content').val($('#pen').html()); pen.destroy();" method="post">
    ...
    <form:input type="text" path="title" placeholder="Title"
                style="height: 50px; width: 100%; font-size: 32px;
    ...
    <form:input type="text" path="subtitle" placeholder="Subtitle (option)"
                style="height: 30px; width: 100%; font-size: 24px;
        border: none; border-right: 0px; border-top: 0px; border-left: 0px; border-bottom: 1px; outline-style: none;
        font-family: 'Open Sans', 'Helvetica Neue', Helvetica, Arial, sans-serif;" />
